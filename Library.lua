...
do
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        local SwitchThumb = Library:Create('Frame', {
            ...
            Position = Toggle.Value and UDim2.new(1, -15, 0.5, -6) or UDim2.new(0, 3, 0.5, -6);
        ...
        function Toggle:Display()
            ...
            local targetPos = Toggle.Value and UDim2.new(1, -15, 0.5, -6) or UDim2.new(0, 3, 0.5, -6);
            Tween(SwitchTrack, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { BackgroundColor3 = trackCol });
            Tween(SwitchThumb, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Position = targetPos, BackgroundColor3 = thumbCol });
        end
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        Library:Create('UIStroke', { Color = Library.OutlineColor, Transparency = 0.15, Thickness = 1, Parent = Track });
        ...
        Track.MouseEnter:Connect(function() Tween(Knob, TI_FAST, { Size = UDim2.new(0, 14, 0, 14) }); end);
        Track.MouseLeave:Connect(function() Tween(Knob, TI_FAST, { Size = UDim2.new(0, 12, 0, 12) }); end);
        ...
        function Dropdown:BuildDropdownList()
            ...
            for _, val in next, Dropdown.Values do
                ...
                Btn.MouseEnter:Connect(function()
                    ...
                    Tween(Btn, TI_FAST, { BackgroundTransparency = isSelected and 0.4 or 0.75 });
                end);
                Btn.MouseLeave:Connect(function()
                    local isSelected = Info.Multi and Dropdown.Value[val] or (Dropdown.Value == val);
...
function Library:CreateWindow(...)
    ...
    Library:Create('UIStroke', { Color = Library.OutlineColor, Transparency = 0.1, Thickness = 1, Parent = TopDock });
    ...
    function Window:AddTab(Name)
        ...
        local Tab = { Groupboxes = {}; Tabboxes = {}; IsOpen = (windowIndex == 1); Active = (windowIndex == 1); };
        ...
        local function RefreshPill(Hovered)
            if Tab.Active then
                Tween(PillStroke, TI_SMOOTH, { Color = Library.AccentColor, Transparency = Hovered and 0 or 0.25 });
                Tween(PillIcon, TI_SMOOTH, { TextColor3 = Library.AccentColor });
                Tween(PillName, TI_SMOOTH, { TextColor3 = Library.FontColor });
                Tween(TabPill, TI_SMOOTH, { BackgroundTransparency = Hovered and 0.85 or 0.92 });
            else
                Tween(PillStroke, TI_SMOOTH, { Color = Library.OutlineColor, Transparency = Hovered and 0 or 1 });
                Tween(PillIcon, TI_SMOOTH, { TextColor3 = Hovered and Library.FontColor or Library.DimColor });
                Tween(PillName, TI_SMOOTH, { TextColor3 = Library.FontColor });
                Tween(TabPill, TI_SMOOTH, { BackgroundTransparency = Hovered and 0.35 or 1 });
            end
        end
        ...
        TabPill.MouseEnter:Connect(function()
            hoveringPill = true;
            Tween(TabPill, TI_SOFT, { Size = UDim2.new(0, ExpandedW, 1, 0) });
            PillName.Visible = true;
            RefreshPill(true);
            Tween(PillName, TI_SMOOTH, { TextTransparency = 0 });
        end);
        ...
        function Tab:AddGroupbox(Info)
            ...
            Library:Create('UIStroke', { Color = Library.OutlineColor, Transparency = 0, Thickness = 1, Parent = BoxOuter });
