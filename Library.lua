...
do
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        local TrackStroke = Library:Create('UIStroke', {
            Color = Toggle.Value and Library.AccentColor or Library.OutlineColor,
            Transparency = Toggle.Value and 0.2 or 0.5,
            Thickness = 1,
            Parent = SwitchTrack,
        });
        ...
        function Toggle:Display()
            ...
            Tween(TrackStroke, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Color = trackCol, Transparency = Toggle.Value and 0.2 or 0.5 });
            Tween(SwitchThumb, TweenInfo.new(0.18, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Position = targetPos, BackgroundColor3 = thumbCol });
...
function WebManager:Update()
    ...
    if #openTabs >= 2 then
        ...
        for i = 1, #openTabs do
            for j = i + 1, #openTabs do
                local tabA = openTabs[i];
                local tabB = openTabs[j];
...
function Library:CreateWindow(...)
    ...
    local function FitDockWidth()
        ...
        local w = math.floor(math.clamp(HeaderW + contentW + 90, 560, maxW));
    ...
    function Window:AddTab(Name)
        ...
        function Tab:OpenWindow()
            ...
            if not Tab.HasOpened then
                ...
                local cascade = (windowIndex - 1) % 8;
                ModWindow.Position = UDim2.fromOffset(
                    math.clamp(70 + cascade * 44, 8, math.max(8, vpX - 470)),
                    math.clamp(90 + cascade * 32, 60, math.max(60, vpY - 570)));
            end
