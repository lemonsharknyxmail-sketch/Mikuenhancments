...
local Library = {
    ...
    BackgroundColor = Color3.fromRGB(13, 14, 18);
    AccentColor = Color3.fromRGB(0, 225, 255);
    OutlineColor = Color3.fromRGB(38, 42, 54);
    RiskColor = Color3.fromRGB(255, 60, 80);
...
function Library:AddToolTip(InfoStr, HoverInstance)
    ...
    local Tooltip = Library:Create('Frame', {
        ...
        Size = UDim2.fromOffset(X + 14, Y + 10),
        ZIndex = 5000,
        Visible = false,
        Parent = ScreenGui,
    });
    Library:Create('UICorner', { CornerRadius = UDim.new(0, 5), Parent = Tooltip });
...
function Library:OnHighlight(HighlightInstance, TargetInstance, Properties, PropertiesDefault)
    HighlightInstance.MouseEnter:Connect(function()
        ...
        for Property, ColorIdx in next, Properties do
            ...
            Tween(TargetInstance, TweenInfo.new(0.2), { [Property] = col });
...
do
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        Library:GiveSignal(InputService.InputBegan:Connect(function(Input)
            if not Picking and KeyPicker.Mode == 'Toggle' then
                local Key = KeyPicker.Value;
...
do
    ...
    function Funcs:AddButton(...)
        ...
        Outer.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame() then
                Tween(Outer, TweenInfo.new(0.08), { Size = UDim2.new(1, -12, 0, 22) });
            end
        end);
        Outer.InputEnded:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                Tween(Outer, TweenInfo.new(0.12, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Size = UDim2.new(1, -8, 0, 24) });
    ...
    function Funcs:AddInput(Idx, Info)
        ...
        Box.Focused:Connect(function() Tween(BoxStroke, TweenInfo.new(0.2), { Color = Library.AccentColor }); end);
        Box.FocusLost:Connect(function() Tween(BoxStroke, TweenInfo.new(0.2), { Color = Library.OutlineColor }); end);
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        function Dropdown:BuildDropdownList()
            ...
            for _, val in next, Dropdown.Values do
                ...
                Btn.MouseEnter:Connect(function() Tween(Btn, TweenInfo.new(0.15), { BackgroundTransparency = 0, TextColor3 = Library.AccentColor }); end);
...
do
    ...
    Library:Create('UIStroke', { Color = Library.AccentColor, Thickness = 1.2, Parent = WatermarkOuter });
...
function Library:Notify(Text, Time)
    ...
    Tween(NotifyCard, TweenInfo.new(0.25), { BackgroundTransparency = 0 });
    Tween(Stroke, TweenInfo.new(0.25), { Transparency = 0 });
    Tween(NotifyLabel, TweenInfo.new(0.25), { TextTransparency = 0 });

    task.delay(Time, function()
        Tween(NotifyCard, TweenInfo.new(0.25), { BackgroundTransparency = 1 });
        Tween(Stroke, TweenInfo.new(0.25), { Transparency = 1 });
        Tween(NotifyLabel, TweenInfo.new(0.25), { TextTransparency = 1 });
        task.wait(0.3);
        NotifyCard:Destroy();
    end);
end
...
function Library:Loader(Info)
    ...
    task.spawn(function()
        ...
        Library:Create('UICorner', { CornerRadius = UDim.new(0, 8), Parent = LoaderGui });
        ...
        Tween(LoaderGui, TweenInfo.new(0.3), { BackgroundTransparency = 1 });
        task.wait(0.35);
        LoaderGui:Destroy();
    end);
end
...
function Library:CreateWindow(...)
    ...
    Library:Create('UICorner', { CornerRadius = UDim.new(0, 10), Parent = TopDock });
    Library:Create('UIStroke', { Color = Library.AccentColor, Thickness = 1.2, Parent = TopDock });
    ...
    local TabScroll = Library:Create('ScrollingFrame', {
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
        Position = UDim2.new(0, 105, 0, 6),
        Size = UDim2.new(1, -175, 1, -12),
    ...
    TabLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
        ...
        TopDock.Size = UDim2.fromOffset(math.clamp(TabLayout.AbsoluteContentSize.X + 280, 520, 900), 42);
    end);
    ...
    Library:Create('UICorner', { CornerRadius = UDim.new(0, 5), Parent = CloseAllBtn });
    local CloseAllStroke = Library:Create('UIStroke', { Color = Library.OutlineColor, Thickness = 1, Parent = CloseAllBtn });
    ...
    function Window:AddTab(Name)
        ...
        local Tab = { Groupboxes = {}; Tabboxes = {}; IsOpen = (windowIndex <= 2); };

        local TabWidth = Library:GetTextBounds(Name, Library.Font, 12) + 20;

        local TabPill = Library:Create('TextButton', {
            BackgroundColor3 = Tab.IsOpen and Library.AccentColor or Library.MainColor,
            Size = UDim2.new(0, TabWidth, 1, 0),
            Font = Library.Font,
            Text = Name,
            TextColor3 = Tab.IsOpen and Color3.new(0, 0, 0) or Library.FontColor,
            TextSize = 12,
            ZIndex = 103,
            Parent = TabScroll,
        });
        Library:Create('UICorner', { CornerRadius = UDim.new(0, 6), Parent = TabPill });
        local PillStroke = Library:Create('UIStroke', { Color = Tab.IsOpen and Library.AccentColor or Library.OutlineColor, Thickness = 1, Parent = TabPill });

        -- Floating Modular Window for this Tab
        local screenW, screenH = workspace.CurrentCamera and workspace.CurrentCamera.ViewportSize.X or 1920, workspace.CurrentCamera and workspace.CurrentCamera.ViewportSize.Y or 1080;
        local staggerX = 80 + ((windowIndex - 1) % 3) * 360;
        local staggerY = 75 + math.floor((windowIndex - 1) / 3) * 60;

        local ModWindow = Library:Create('Frame', {
            Position = UDim2.fromOffset(staggerX, staggerY),
        ...
        Library:Create('UICorner', { CornerRadius = UDim.new(0, 10), Parent = ModWindow });
        local ModWindowStroke = Library:Create('UIStroke', { Color = Library.OutlineColor, Thickness = 1.5, Parent = ModWindow });
        ...
        Library:Create('UICorner', { CornerRadius = UDim.new(0, 10), Parent = WinHeader });
        ...
        CloseBtn.MouseEnter:Connect(function() Tween(CloseBtn, TweenInfo.new(0.15), { TextColor3 = Library.RiskColor }); end);
        CloseBtn.MouseLeave:Connect(function() Tween(CloseBtn, TweenInfo.new(0.15), { TextColor3 = Color3.fromRGB(160, 160, 170) }); end);
        ...
        function Tab:OpenWindow()
            ...
            Tween(TabPill, TweenInfo.new(0.2), { BackgroundColor3 = Library.AccentColor, TextColor3 = Color3.new(0, 0, 0) });
            Tween(PillStroke, TweenInfo.new(0.2), { Color = Library.AccentColor });
            Tween(ModWindowStroke, TweenInfo.new(0.25), { Color = Library.AccentColor });
            task.delay(0.4, function()
                if Tab.IsOpen then Tween(ModWindowStroke, TweenInfo.new(0.25), { Color = Library.OutlineColor }); end
            end);
        end
        ...
        function Tab:CloseWindow()
            ...
            Tween(TabPill, TweenInfo.new(0.2), { BackgroundColor3 = Library.MainColor, TextColor3 = Library.FontColor });
            Tween(PillStroke, TweenInfo.new(0.2), { Color = Library.OutlineColor });
        end
        ...
        function Tab:AddTabbox(Info)
            ...
            function Tabbox:AddTab(SubTabName)
                ...
                function SubTab:Hide()
                    ...
                    Tween(Button, TweenInfo.new(0.2), { BackgroundColor3 = Library.BackgroundColor, TextColor3 = Library.FontColor });
                ...
                function SubTab:Resize()
                    ...
                    for _, child in next, TabboxButtons:GetChildren() do
                        if child:IsA('TextButton') then child.Size = UDim2.new(1 / TabCount, -4, 1, 0); end
                    end
