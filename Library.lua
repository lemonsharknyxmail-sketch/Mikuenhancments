...
do
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        local CheckInner = Library:Create('Frame', {
            ...
            Size = Toggle.Value and UDim2.new(0, 8, 0, 8) or UDim2.new(0, 0, 0, 0);
            BackgroundTransparency = Toggle.Value and 0 or 1;
            ZIndex = 4;
            Parent = CheckOuter;
        });
        Library:Create('UICorner', { CornerRadius = UDim.new(0, 2), Parent = CheckInner });
        ...
        Library:AddToRegistry(CheckInner, {
        ...
        function Toggle:Display()
            ...
            local innerSize = Toggle.Value and UDim2.new(0, 8, 0, 8) or UDim2.new(0, 0, 0, 0);
...
function Library:CreateWindow(...)
    ...
    WebManager:Init(Window);
        ...
        -- Top Navigation Dock Bar (Sleek & Compact)
    ...
    local DockStroke = Library:Create('UIStroke', { Color = Library.OutlineColor, Transparency = 0.2, Thickness = 1, Parent = TopDock });
    Library:AddToRegistry(TopDock, { BackgroundColor3 = 'BackgroundColor' });
    Library:AddToRegistry(DockStroke, { Color = 'OutlineColor' });

    -- TopDock Subtle Shadow
    ...
    local TitleW = Library:GetTextBounds(Config.Title, Library.Font, 12);
    local HeaderW = math.max(76, 24 + TitleW + 10);

    local DockHeader = Library:Create('Frame', {
        BackgroundTransparency = 1,
        Position = UDim2.new(0, 10, 0, 0),
        Size = UDim2.new(0, HeaderW - 10, 1, 0),
        ZIndex = 31,
        Parent = TopDock,
    });
    ...
    task.spawn(function()
        ...
        while PulsingDot.Parent do
            ...
            Tween(PulsingDot, TweenInfo.new(1.1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
                Size = grow and UDim2.fromOffset(5, 5) or UDim2.fromOffset(7, 7);
                BackgroundTransparency = grow and 0.4 or 0;
            });
            task.wait(1.1);
        end
    end);
    ...
    Library:Create('Frame', {
        ...
        BorderSizePixel = 0,
        Position = UDim2.new(0, HeaderW + 2, 0, 8),
    ...
    local TabScroll = Library:Create('ScrollingFrame', {
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
        Position = UDim2.new(0, HeaderW + 8, 0, 4),
        Size = UDim2.new(1, -(HeaderW + 116), 1, -8),
    ...
    local TabLayout = Library:Create('UIListLayout', {
        ...
        local lastFitW = -1;
    local isFitting = false;
    local function FitDockWidth()
        if isFitting then return end
        local contentW = TabLayout.AbsoluteContentSize.X;
        if contentW <= 0 or contentW == lastFitW then return end
        lastFitW = contentW;
        isFitting = true;
        task.defer(function()
            isFitting = false;
            if not TopDock or not TopDock.Parent or not TabLayout or not TabLayout.Parent then return end
            local curW = TabLayout.AbsoluteContentSize.X;
            local cam = workspace.CurrentCamera;
            local maxW = cam and (cam.ViewportSize.X - 30) or 800;
            local w = math.floor(math.clamp(HeaderW + curW + 124, 460, maxW));
            TopDock.Size = UDim2.fromOffset(w, 36);
            TabScroll.CanvasSize = UDim2.fromOffset(curW + 8, 0);
        end);
    end
    ...
    local ArrangeStroke = Library:Create('UIStroke', { Color = Library.OutlineColor, Transparency = 0.3, Thickness = 1, Parent = ArrangeBtn });
    ...
    ArrangeBtn.MouseLeave:Connect(function()
        ...
        Tween(ArrangeStroke, TI_FAST, { Transparency = 0.3 });
    end);
    ...
    local CloseAllStroke = Library:Create('UIStroke', { Color = Library.OutlineColor, Transparency = 0.3, Thickness = 1, Parent = CloseAllBtn });
    ...
    CloseAllBtn.MouseLeave:Connect(function()
        ...
        Tween(CloseAllStroke, TI_FAST, { Transparency = 0.3 });
    end);
    ...
    function Window:ArrangeWindows()
        ...
        for i, tab in ipairs(openTabs) do
            ...
            local targetY = math.clamp(startY + row * (winH + gapY), 50, math.max(50, vpY - winH - 8));
    ...
    function Window:AddTab(Name)
        ...
        local Tab = {
            ...
            Active = (Window.AutoShow and windowIndex == 1);
                ...
                local NameW = Library:GetTextBounds(Name, Library.Font, 11);
        local PillW = NameW + 16;
        ...
        local function RefreshPill(Hovered)
            if Tab.IsOpen then
                ...
                Tween(Underline, TI_SOFT, { Size = UDim2.new(0.5, 0, 0, 2), BackgroundTransparency = 0.05 });
            elseif Hovered then
        ...
        -- Floating Modular Window for this Tab (Sleek Proportional Dimensions)
        ...
        function Tab:AddTabbox(Info)
            ...
            function Tabbox:AddTab(SubTabName)
                ...
                function SubTab:Show()
                    ...
                    Tween(Button, TI_SMOOTH, { BackgroundColor3 = Library.AccentColor, BackgroundTransparency = 0.85, TextColor3 = Library.FontColor });
        ...
        if Tab.Active then
            ...
            PillName.TextColor3 = Library.FontColor;
    ...
    -- Web mesh disabled for performance
...
-- 1. TARGET HUD (Cyber Luxury Redesign)
do
    local TargetHUD = {
        Frame = nil,
        Enabled = false,
    ...
    function TargetHUD:Init()
        ...
        local CardStroke = Library:Create('UIStroke', { Color = Library.OutlineColor, Transparency = 0.15, Thickness = 1.2, Parent = Card });
        ...
        -- Damage Lag Bar (Soft Red Animated Lag Fill)
        ...
        -- Main Health Fill (Dynamic Accent Gradient)
    ...
    function TargetHUD:Update(info)
        ...
        self:Init();
        if not info then
            if self.Frame.Visible and (tick() - self.LastSeen) > 1.2 then
        ...
        -- Instant main bar tween + Smooth trailing damage lag bar
        Tween(self.HpFill, TI_FAST, { Size = UDim2.new(hpFrac, 0, 1, 0) });
        if hpFrac < self.CurrentHpFrac then
            -- Damage taken: animate lag bar with delay
